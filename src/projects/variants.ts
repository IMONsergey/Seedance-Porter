...
export function planVariants(input: unknown, options: { count: number; seedStart?: number; provider?: ProviderName }) {
  ...
  if (!model.supportsSeed) throw new PorterError("UNSUPPORTED", `${model.displayName} does not advertise seed control.`);
  ...
  return variants.map((project) => ({ project, compiled: compileProject(project, options.provider) }));
}
