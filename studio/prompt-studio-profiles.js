export const PROMPT_STUDIO_PROFILES = Object.freeze([
  {
    ...
    description:'Balanced production policy for most Seedance prompts.',
...
const PROFILE_RULES = new Set(PROMPT_STUDIO_PROFILES.flatMap(profile => profile.rules));
...
export function applyPromptStudioProfile(project, profileId) {
  ...
  const userRules = (project.customRules || []).filter(rule => !PROFILE_RULES.has(rule));
  return {
...
export function getPromptStudioUserRules(project) {
  return (project.customRules || []).filter(rule => !PROFILE_RULES.has(rule));
}

export function getPromptStudioActiveProfileRules(project) {
  const profile = getPromptStudioProfile(project.modelProfile);
  const current = new Set(project.customRules || []);
  return profile.rules.filter(rule => current.has(rule));
}

export function getPromptStudioProfileCoverage(project) {
  const profile = getPromptStudioProfile(project.modelProfile);
  const current = new Set(project.customRules || []);
  return {
...
export function promptStudioProfileRuleSet() {
  return new Set(PROFILE_RULES);
}
