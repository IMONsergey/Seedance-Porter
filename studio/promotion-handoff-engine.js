...
import { auditDeepReviewQuality } from './review-quality-engine.js';

export const PROMOTION_HANDOFF_KIND = 'seedance-porter-promotion-handoff';
export const PROMOTION_QUALITY_MINIMUM = 76;
...
export async function buildPromotionHandoffFromEvidence(input) {
  ...
  const qualityAudit = auditDeepReviewQuality(deepReview, { expectedCandidateId:String(pkg.candidateId || ''), now:createdAt });
  const qualityHash = await hashJson(qualityAudit);
  const qualityReady = qualityAudit.gate === 'quality-pass' && Number(qualityAudit.score || 0) >= PROMOTION_QUALITY_MINIMUM;
  const qualityReviewRequired = !qualityReady;
  ...
  if (qualityReviewRequired) warnings.push(`Deep Review quality gate requires attention: ${qualityAudit.gate}, ${qualityAudit.score}/100, grade ${qualityAudit.grade}.`);
  if (riskReviewRequired) warnings.push(`Research risk review required before curation: ${riskFlags.join(', ')}`);

  const status = handoffStatus({ qualityAudit, qualityReviewRequired, riskReviewRequired });
  const promotionInputReady = qualityReady && !riskReviewRequired && Boolean(sourceUrl) && deepReview.reviewStatus === 'deep-reviewed';
  const handoffCore = {
    schemaVersion:2,
    kind:PROMOTION_HANDOFF_KIND,
    createdAt,
    ...
    reviewQuality:{
      ...qualityAudit,
      minimumScore:PROMOTION_QUALITY_MINIMUM,
      promotionReady:qualityReady,
      auditHash:qualityHash
    },
    candidate,
    ...
    autoPublish:false,
    autoGitHubWrite:false,
    boundary:'This handoff unwraps verified evidence for Promotion review only after the Deep Review quality gate passes. It does not clear Research risk flags, approve rights/attribution, satisfy the Promotion editorial gate, create a Curation Draft, rotate the top-100, publish, or write to GitHub.'
  };
...
export async function verifyPromotionHandoff(value) {
  ...
  const quality = handoff.reviewQuality || null;
  if (!quality) {
    errors.push('Promotion handoff must include a Deep Review quality audit.');
  } else {
    const recomputed = auditDeepReviewQuality(handoff.promotionReviewInput || {}, {
      expectedCandidateId:handoff.candidateId,
      now:quality.auditedAt || handoff.createdAt
    });
    const expectedQualityHash = await hashJson(recomputed);
    if (quality.auditHash !== expectedQualityHash) errors.push('Deep Review quality audit hash mismatch.');
    if (quality.score !== recomputed.score || quality.grade !== recomputed.grade || quality.gate !== recomputed.gate) errors.push('Deep Review quality audit does not match Promotion review input.');
    const qualityReady = recomputed.gate === 'quality-pass' && recomputed.score >= Number(quality.minimumScore || PROMOTION_QUALITY_MINIMUM);
    if (handoff.promotionInputReady === true && !qualityReady) errors.push('Promotion input cannot be ready when the Deep Review quality gate has not passed.');
    if (handoff.status === 'ready-for-promotion-review' && !qualityReady) errors.push('ready-for-promotion-review requires quality-pass.');
    if (quality.promotionReady !== qualityReady) errors.push('reviewQuality.promotionReady is inconsistent with the recomputed audit.');
  }
...
export function renderPromotionHandoffHtml(handoff){
  ...
  const quality=handoff.reviewQuality||{};
