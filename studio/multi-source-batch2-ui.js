import { MULTI_SOURCE_CASES_BATCH_2 as CASES } from './multi-source-cases-batch2.js';
...
function cardHtml(item) {
  ...
  return `<article class="prompt-card digest-card multi-source-card batch2-card" data-batch2-id="${attr(item.id)}" tabindex="0">
    <div class="card-preview source-preview">
      <div class="multi-source-poster"><span class="multi-source-poster-platform">${esc(p.label)}</span><strong>${esc(loc(item,'title'))}</strong><small>${esc(item.author)} · ${ui('curated craft case','curated craft-кейс')}</small></div>
      <button class="media-play-button" type="button" data-batch2-open="${attr(item.id)}"><span>▶</span><span>${ui('Play video','Смотреть видео')}</span></button>
    </div>
    <div class="card-body">
      <div class="card-kicker"><span class="platform-badge">${esc(p.label)} · ${item.sourceKind === 'award-case' ? ui('Award case','Награждённый кейс') : item.sourceKind === 'motion-reference' ? ui('Motion reference','Motion-референс') : ui('Workflow case','Разбор workflow')}</span><span>${ui('design','дизайн')} ${item.designScore}/5</span></div>
      ...
      <div class="card-actions"><button type="button" data-batch2-copy="${attr(item.id)}">${ui('Copy Porter version','Скопировать Porter-версию')}</button><button type="button" data-batch2-open="${attr(item.id)}">${ui('Open case','Открыть кейс')}</button></div>
    </div>
  </article>`;
}
...
function render() {
  ...
  const existing = $$('[data-batch2-id]',grid);
  const have = existing.map(el=>el.dataset.batch2Id).join('|');
  ...
  if (have !== want) {
    ...
    existing.forEach(el => {
      const item = byId.get(el.dataset.batch2Id);
      if (!item) return;
      const title = $('h3',el); if(title) title.textContent = loc(item,'title');
      const why = $('.excerpt',el); if(why) why.textContent = loc(item,'why');
      const play = $('[data-batch2-open] span:last-child',el); if(play) play.textContent = ui('Play video','Смотреть видео');
...
function resolvedPrompt(item, root) {
  ...
  $$('[data-batch2-variable]',root).forEach(input => { values[input.dataset.batch2Variable] = input.value; });
...
function drawerHtml(item) {
  ...
  return `${playerHtml(item)}
    <div class="multi-drawer-sourcebar"><span>${esc(p.label)} · ${item.sourceKind === 'award-case'?ui('Award case','Награждённый кейс'):item.sourceKind==='motion-reference'?ui('Motion reference','Motion-референс'):ui('Workflow case','Разбор workflow')}</span><a href="${attr(item.sourceUrl)}" target="_blank" rel="noopener">${ui('Source case','Кейс-источник')} ↗</a></div>
    <div class="drawer-body" data-batch2-body>
      ...
      <div class="variable-grid">${vars.map(([key,value])=>`<label>${esc(key.replace(/([A-Z])/g,' $1'))}<input data-batch2-variable="${attr(key)}" value="${attr(value)}" /></label>`).join('')}</div>

      <div class="section-title"><span>Porter Adaptation</span><span>${ui('independently written','самостоятельно написано')}</span></div>
      <div class="prompt-box adaptation-box" data-batch2-prompt>${esc(item.porterPrompt)}</div>
      <div class="drawer-actions"><button class="button primary" type="button" data-batch2-copy-resolved>${ui('Copy Porter Adaptation','Скопировать Porter-адаптацию')}</button><button class="button" type="button" data-batch2-project>${ui('Copy Porter project JSON','Скопировать Porter project JSON')}</button><a class="button" href="${attr(item.sourceUrl)}" target="_blank" rel="noopener">${ui('Open source','Открыть источник')} ↗</a></div>
    </div>`;
}
...
function openCase(item) {
  ...
  const body=$('[data-batch2-body]',content);
  const update=()=>{ const box=$('[data-batch2-prompt]',body); if(box) box.textContent=resolvedPrompt(item,body); };
  $$('[data-batch2-variable]',body).forEach(input=>input.addEventListener('input',update));
  $('[data-batch2-copy-resolved]',body)?.addEventListener('click',()=>copy(resolvedPrompt(item,body),ui('Porter Adaptation copied','Porter-адаптация скопирована')));
  $('[data-batch2-project]',body)?.addEventListener('click',()=>copy(JSON.stringify(projectJson(item,resolvedPrompt(item,body)),null,2),ui('Project JSON copied','Project JSON скопирован')));
}

function bind() {
  const grid=$('#digestGrid');
  grid?.addEventListener('click',event=>{
    const copyButton=event.target.closest('[data-batch2-copy]');
    if(copyButton){ event.stopPropagation(); const item=byId.get(copyButton.dataset.batch2Copy); if(item) copy(item.porterPrompt,ui('Porter Adaptation copied','Porter-адаптация скопирована')); return; }
    const openButton=event.target.closest('[data-batch2-open]');
    if(openButton){ event.stopPropagation(); const item=byId.get(openButton.dataset.batch2Open); if(item) openCase(item); }
  });
  grid?.addEventListener('keydown',event=>{ if((event.key==='Enter'||event.key===' ')&&event.target.matches('[data-batch2-id]')){ const item=byId.get(event.target.dataset.batch2Id); if(item) openCase(item); }});
