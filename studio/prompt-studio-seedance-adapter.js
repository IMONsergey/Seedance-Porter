...
export async function buildSeedance2ModelArkExport(handoff,options={}){
  ...
  const cleanErrors=[...new Set(errors)],cleanWarnings=[...new Set(warnings)];const ready=cleanErrors.length===0;const studioLink=buildStudioLink(handoff);
  return{kind:'seedance-porter-provider-export',schemaVersion:1,provider:'byteplus-modelark',adapter:'seedance-2.0',verifiedAgainst:{profile:SEEDANCE2_MODELARK_PROFILE.id,verifiedAt:SEEDANCE2_MODELARK_PROFILE.verifiedAt,apiReferenceUpdatedAt:SEEDANCE2_MODELARK_PROFILE.apiReferenceUpdatedAt},studioLink,endpoint:SEEDANCE2_MODELARK_PROFILE.endpoint,retrieveEndpoint:SEEDANCE2_MODELARK_PROFILE.retrieveEndpoint,ready,errors:cleanErrors,warnings:cleanWarnings,payload:ready?payload:null,previewPayload:payload,referenceTokenMap:Object.fromEntries(tokenMap),policy:{autoSubmit:false,apiKeyEmbedded:false,networkRequest:false,requiresExternalExecution:true}};
}

export function seedance2ExportToCurl(exportBundle){if(!exportBundle?.previewPayload)throw new Error('Seedance export bundle is required.');if(!exportBundle.ready)throw new Error('Seedance export is blocked; resolve preflight errors before creating an execution snippet.');return`curl ${SEEDANCE2_MODELARK_PROFILE.endpoint} \\
