...
export function auditPromptStudioGenerationDecisions(project){
  ...
  for(const raw of rawEvaluations){
    const taskId=String(raw?.taskId||''),exportHash=normalizeHash(raw?.exportHash),verdict=String(raw?.verdict||'');
    if(!taskId)issues.push(issue('error','evaluation-task-empty',raw?.id,'Raw Evaluation has no task ID.'));
    if(taskId&&rawEvaluationTasks.has(taskId))issues.push(issue('error','evaluation-task-duplicate',taskId,'More than one raw Evaluation exists for the same generation task.'));if(taskId)rawEvaluationTasks.add(taskId);
    if(!/^[a-f0-9]{64}$/.test(exportHash))issues.push(issue('error','evaluation-export-invalid',taskId||raw?.id,'Raw Evaluation has an invalid export hash.'));
    if(verdict&&!VERDICTS.has(verdict))issues.push(issue('error','evaluation-verdict-invalid',taskId||raw?.id,`Unsupported Evaluation verdict: ${verdict}.`));
  }
  ...
  const rawComparisonIds=new Set();
  for(const raw of rawComparisons){
    const id=String(raw?.id||''),ids=Array.isArray(raw?.taskIds)?raw.taskIds.map(String).filter(Boolean):[],unique=[...new Set(ids)];
    if(!id)issues.push(issue('error','comparison-id-empty','generationComparisons','Raw Comparison has no ID.'));
    if(id&&rawComparisonIds.has(id))issues.push(issue('error','comparison-id-duplicate',id,'Comparison ID is duplicated in raw extension state.'));if(id)rawComparisonIds.add(id);
    if(ids.length<2||ids.length>8||unique.length!==ids.length)issues.push(issue('error','comparison-task-set-invalid',id||'comparison',`Comparison must contain 2–8 unique task IDs; received ${ids.length}.`));
  }
  ...
  for(const raw of rawRetakes){
    const id=String(raw?.id||''),taskId=String(raw?.sourceTaskId||''),lever=String(raw?.lever||'');
    if(!id)issues.push(issue('error','retake-id-empty',taskId,'Raw Retake Draft has no ID.'));
    if(id&&rawRetakeIds.has(id))issues.push(issue('error','retake-id-duplicate',id,'Retake Draft ID is duplicated.'));if(id)rawRetakeIds.add(id);
    if(!RETAKE_LEVERS.has(lever))issues.push(issue('error','retake-lever-invalid',id||taskId,`Unsupported Retake lever: ${lever||'(empty)'}.`));
    const record=byTask.get(taskId);if(!record){issues.push(issue('error','retake-source-missing',taskId||id,'Retake Draft references a generation task that no longer exists.'));continue;}
    if(record.status!=='succeeded'||!record.output?.videoUrl)issues.push(issue('error','retake-source-not-succeeded',taskId,'Retake Draft must start from a succeeded visual generation.'));
    if(normalizeHash(record.exportHash)!==normalizeHash(raw?.sourceExportHash))issues.push(issue('error','retake-export-drift',taskId,'Retake source export hash no longer matches the generation record.'));
    if(!String(raw?.changeInstruction||'').trim())issues.push(issue('error','retake-instruction-empty',id,'Retake Draft has no explicit change instruction.'));
    if(!String(raw?.expectedImprovement||'').trim())issues.push(issue('error','retake-expected-improvement-empty',id,'Retake Draft must define expected improvement.'));
    if(!Array.isArray(raw?.retainedLocks)||!raw.retainedLocks.map(String).some(value=>value.trim()))issues.push(issue('error','retake-locks-empty',id,'Retake Draft must explicitly preserve at least one successful lock.'));
    const evaluation=evaluations.find(item=>item.taskId===taskId);if(!evaluation)issues.push(issue('error','retake-evaluation-missing',taskId,'Retake Draft source has no saved Evaluation.'));else if(evaluation.ratedDimensions<1||evaluation.evidenceDimensions<1)issues.push(issue('error','retake-evaluation-evidence-thin',taskId,'Retake Draft source Evaluation needs at least one rated and one evidence-covered dimension.'));
  }
