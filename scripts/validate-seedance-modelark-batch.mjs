...
// Ambiguous POST: one network failure before a task ID is known must never auto-retry.
...
// Known task ID: poll interruption may resume without another POST.
...
if(failures.length){console.error('Seedance ModelArk Batch Runner contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,items:2,configuredConcurrency:2,observedMaxConcurrency:maxActive,paidPosts:postCount,submissionUncertainNoRetry:true,knownTaskResumeWithoutPost:true,credentialPersistence:false,batchResultValid:true},null,2));
