...
export async function retrieveSeedanceGeneration(job,{requester=globalThis.fetch,apiKey=process.env.ARK_API_KEY,now=Date.now()}={}){
  ...
  const response=await request(job.retrieveUrl,{method:'GET',headers:authHeaders(key)});const payload=await readJsonResponse(response,'retrieve',{redact:[key]});
...
export async function cancelQueuedSeedanceGeneration(job,{requester=globalThis.fetch,apiKey=process.env.ARK_API_KEY,now=Date.now()}={}){
  ...
  const response=await request(current.retrieveUrl,{method:'DELETE',headers:authHeaders(key)});await readJsonResponse(response,'cancel',{emptyObjectAllowed:true,redact:[key]});const updated=iso(now);
