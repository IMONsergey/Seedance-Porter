...
assert(refs[0].token==='@ref01'&&refs[0].legacyNumber===1,'First legacy image must retain legacyNumber=1 for token rewrite.');
assert(refs[1].token==='@ref02'&&refs[1].legacyNumber===2,'Second legacy video must retain legacyNumber=2 for token rewrite.');
...
console.log(JSON.stringify({ok:true,legacyReferenceRewrite:true,cameraMoveCount:twoMoveLint.metrics.cameraMoves,researchRiskLint:true,schemaShape:'13 canonical sections'},null,2));
