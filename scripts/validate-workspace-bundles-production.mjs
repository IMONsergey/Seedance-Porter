...
for(const asset of ['workspace-bundle.css','workspace-bundle-bootstrap.js','workspace-bundle-engine.js','workspace-bundle-ui.js'])assert(pages.includes(`cp studio/${asset} _site/${asset}`),`Pages must publish ${asset}.`);
assert(pages.includes('node scripts/validate-workspace-bundles.mjs'),'Pages must run Workspace Bundle safety contract.');
assert(pages.includes('node scripts/validate-workspace-bundles-production.mjs'),'Pages must run Workspace Bundle production contract.');
...
console.log(JSON.stringify({ok:true,curatedCases:curated.length,transportOnly:true,autoApproval:false,autoGitHubWrite:false},null,2));
