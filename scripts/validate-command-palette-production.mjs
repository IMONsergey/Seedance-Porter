...
assert(pages.includes('node scripts/validate-command-palette.mjs'),'Pages must run Command Palette ranking contract.');
assert(pages.includes('node scripts/validate-command-palette-production.mjs'),'Pages must run Command Palette production contract.');
...
console.log(JSON.stringify({ok:true,curatedCases:curated.length,assets:['command-palette.css','command-palette-bootstrap.js','command-palette-engine.js','command-palette-ui.js'],shortcut:'Cmd/Ctrl+K',curatedMutation:false},null,2));
