...
assert(text.ui.includes("if(document.querySelector('#studioV8BatchDock'))return"),'V8 MutationObserver mount must be idempotent and must not render-loop on its own DOM mutations.');
...
assert(text.batch.includes('provider-project-version-mismatch')&&text.batch.includes('result-project-version-mismatch'),'Batch manifests must semantically cross-check canonical project/version lineage, not only hashes.');
...
assert(text.batchRunner.includes('terminal?applyJobStudioLinkToResult(buildGenerationResult(current,now()),current):null'),'Terminal poll snapshots must materialize result before persistence so crash recovery stays protocol-valid.');
assert(text.batchRunner.includes('if(!actionable.length)')&&text.batchRunner.includes('base.completedAt||iso(now())'),'Re-running a settled batch must stay settled rather than transiently returning to running.');
...
assert(text.batchCli.includes('loadJsonIfExists')&&text.batchCli.includes("error?.code==='ENOENT'")&&!text.batchCli.includes('catch{job=await createSeedanceBatchJob'),'Existing unreadable/corrupt batch job must fail closed; only ENOENT may create a new resumable job.');
assert(text.batchCli.includes('saveJsonAtomic')&&text.batchCli.includes('await rename(temp,target)'),'Batch job/result persistence must use temp-file + rename atomic writes.');
assert(text.batchCli.includes('assertPathMissing')&&text.batchCli.includes("error.code='batch-job-exists'"),'Explicit create must refuse to overwrite an existing resumable batch job.');
assert(text.batchCliTest.includes('duplicateCreateRefused')&&text.batchCliTest.includes('malformedResumeFailClosed'),'Batch CLI behavioral contract must prove existing/corrupt manifests are never silently replaced.');
...
assert(Array.isArray(jobSchema.allOf)&&jobSchema.allOf.length>=2,'Batch Job schema must bind top-level completedAt to terminal/non-terminal batch lifecycle.');
...
assert(projectState.active?.phase==='prompt-studio-v8-variant-batch-generation'&&projectState.active?.branch==='feat/prompt-studio-v8-batch-generation'&&projectState.active?.pullRequest===46,'Machine-readable PROJECT-STATE must identify actual V8 active phase/branch/PR.');
...
if(failures.length){console.error('Prompt Studio v8 production contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,mountOrder:['v6','v7','v8','cmd-k'],browserProviderExecution:false,batchMaxItems:20,localConcurrencyMax:8,ambiguousAutoRetry:false,atomicBatchManifests:true,duplicateCreateRefused:true,corruptJobFailClosed:true,crashConsistentSnapshots:true,settledRerunStable:true,resumableKnownTask:true,singleRunnerLineage:true,monotonicGenerationHistory:true,semanticProjectLineage:true,projectMemoryCanonical:true,curatedCases:curated.length,porterOriginals:PROMPTS.length},null,2));
