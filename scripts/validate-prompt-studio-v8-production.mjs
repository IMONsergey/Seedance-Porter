...
if(failures.length){console.error('Prompt Studio v8 production contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,mountOrder:['v6','v7','v8','cmd-k'],browserProviderExecution:false,batchMaxItems:20,localConcurrencyMax:8,ambiguousAutoRetry:false,atomicBatchManifests:true,corruptJobFailClosed:true,crashConsistentSnapshots:true,resumableKnownTask:true,singleRunnerLineage:true,monotonicGenerationHistory:true,semanticProjectLineage:true,projectMemoryCanonical:true,curatedCases:curated.length,porterOriginals:PROMPTS.length},null,2));
