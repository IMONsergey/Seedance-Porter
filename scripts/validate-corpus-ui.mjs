...
requireText(ui, "data-case-view = 'corpus'".replace(/ = /, ' = '), '');
requireMatch(ui, /data\.caseView\s*=\s*['"]corpus['"]/, 'Research Corpus nav must use an isolated case-view attribute');
...
requireText(ui, 'state.pageSize: 36'.replace(':', ' ='), '');
