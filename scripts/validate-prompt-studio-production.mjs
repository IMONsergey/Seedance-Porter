...
const [pages,sidebar,router,bootstrap,ui,bridge,ai,lifecycle,preview]=await Promise.all([
  ...
  readFile('studio/prompt-studio-asset-lifecycle.js','utf8'),
  readFile('studio/prompt-studio-reference-preview.js','utf8')
]);
const failures=[];
...
assert(bootstrap.includes("await import('./prompt-studio-asset-lifecycle.js')")&&bootstrap.includes("await import('./prompt-studio-reference-preview.js')"),'Prompt Studio bootstrap must mount asset cleanup and preview layers.');
for(const css of ['./prompt-studio.css','./prompt-studio-bridge.css','./prompt-studio-preview.css'])assert(bootstrap.includes(css),`Prompt Studio bootstrap must load ${css}.`);
...
assert(bridge.includes("document.querySelector('[data-command-close]')?.click()"),'Cmd-K bridge must close the palette before opening Studio.');
assert(lifecycle.includes('deletePromptStudioProjectAssets'),'Deleting a Prompt Studio project must clean its IndexedDB media.');
assert(preview.includes('promptStudioAssetObjectUrl'),'Reference preview layer must render local IndexedDB assets.');
assert(preview.includes("document.createElement('video')")&&preview.includes("document.createElement('img')"),'Reference preview layer must support image and video references.');
