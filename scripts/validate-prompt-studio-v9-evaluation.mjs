...
project=savePromptStudioGenerationEvaluation(project,{taskId:'task-push',exportHash:'a'.repeat(64),scores:scores({camera:5,'motion-action':5,'production-readiness':5,continuity:4}),verdict:'candidate',strengths:['clean camera','stable geometry'],weaknesses:['slight lighting softness'],artifactFlags:[],notes:'Best motion read.'},{now:Date.parse('2026-08-08T19:02:00Z')});
...
const pushEval=promptStudioGenerationEvaluationForTask(project,'task-push');assert(pushEval?.overallScore===4.75,'Evaluation overall score must be transparent average of rated dimensions only.');
...
const pushRow=view.rows.find(row=>row.taskId==='task-push');assert(pushRow?.variant?.id==='variant-push','Comparison must recover V8 batch → V4 variant lineage.');
assert(pushRow?.variant?.changedControls?.length>0,'Comparison must expose changed controls from the variant delta.');
assert(pushRow?.overallScore===4.75,'Comparison must surface saved evaluation score without copying evaluation data into comparison record.');

let outsider=false;try{setPromptStudioGenerationWinner(project,'cmp-v9','task-outsider');}catch{outsider=true;}assert(outsider,'Winner selection must reject a task outside the comparison.');
project=setPromptStudioGenerationWinner(project,'cmp-v9','task-push',{reason:'Best camera/motion balance.',now:'2026-08-08T19:04:00.000Z'});
...
view=buildPromptStudioGenerationComparisonView(project,'cmp-v9');assert(view.winner?.reason==='Best camera/motion balance.','Comparison view must surface explicit human winner rationale.');
...
const retake=createPromptStudioGenerationRetake(project,{sourceTaskId:'task-light',lever:'camera',changeInstruction:'Change only the camera: use the precise push-in from the winning take while keeping lighting and subject locks unchanged.',expectedImprovement:'Increase motion readability without losing the stronger edge light.',retainedLocks:['subject geometry','edge-light direction','material response']},{id:'retake-v9',now:'2026-08-08T19:06:00.000Z'});project=retake.project;assert(retake.retake.lever==='camera'&&retake.retake.status==='draft','Retake must record one named production lever as a draft only.');assert(listPromptStudioGenerationRetakes(project).length===1,'Retake Draft must persist extension-safely.');let emptyRetake=false;try{createPromptStudioGenerationRetake(project,{sourceTaskId:'task-light',lever:'camera',changeInstruction:''});}catch{emptyRetake=true;}assert(emptyRetake,'Retake must require an explicit change instruction.');

const refreshed=refreshPromptStudioProject(project,'2026-08-08T19:07:00.000Z');assert(promptStudioGenerationEvaluationForTask(refreshed,'task-push')?.overallScore===4.75,'Core project refresh must preserve V9 evaluations.');assert(promptStudioGenerationWinner(refreshed,'cmp-v9')?.taskId==='task-push','Core project refresh must preserve V9 winner selection.');assert(listPromptStudioGenerationRetakes(refreshed)[0]?.id==='retake-v9','Core project refresh must preserve V9 retake drafts.');

if(failures.length){console.error('Prompt Studio v9 Evaluation contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,dimensions:GENERATION_EVALUATION_DIMENSIONS.length,evaluations:2,comparisonRows:2,variantDeltaVisible:true,humanWinner:true,winnerContinuation:true,oneLeverRetake:true,extensionPersistence:true},null,2));
