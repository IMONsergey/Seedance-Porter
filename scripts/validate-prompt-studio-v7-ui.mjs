...
const dom=new JSDOM(`<!doctype html><html><head></head><body><section id="promptStudioView"><section id="studioV4Dock"><b data-v4-dirty="false"></b></section><section id="studioV5Dock" data-repair-staged="false"></section></section></body></html>`,{url:'https://example.test/'});
...
const dirty=document.querySelector('#studioV4Dock [data-v4-dirty]');dirty.dataset.v4Dirty='true';
...
dirty.dataset.v4Dirty='false';

const v5=document.querySelector('#studioV5Dock');v5.dataset.repairStaged='true';
...
dom.window.close();
if(failures.length){console.error('Prompt Studio v7 UI contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,stagedImport:true,noMutationOnImport:true,exactLineage:true,storyboardGuard:true,repairGuard:true,revisionedAttach:true,projectDriftInvalidation:true,credentialImportBlocked:true,browserFetchCalls:fetchCalls,autoMediaEmbed:false,directProductionSignals:true},null,2));
