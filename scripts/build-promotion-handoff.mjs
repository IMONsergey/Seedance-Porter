...
if(handoff.valid&&handoff.promotionReviewInput)await writeFile(reviewPath,`${JSON.stringify(handoff.promotionReviewInput,null,2)}\n`,'utf8');
await writeFile(htmlPath,renderPromotionHandoffHtml(handoff),'utf8');
console.log(JSON.stringify({valid:handoff.valid,status:handoff.status,promotionInputReady:handoff.promotionInputReady,candidateId:handoff.candidateId,review:handoff.valid?reviewPath:null,context:contextPath,html:htmlPath,riskFlags:handoff.researchRisk?.flags||[],handoffHash:handoff.handoffHash||null},null,2));
if(!handoff.valid)process.exitCode=2;
if(handoff.valid&&!handoff.promotionInputReady)process.exitCode=3;
