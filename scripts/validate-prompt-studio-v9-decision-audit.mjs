...
project=createPromptStudioGenerationComparison(project,['task-a','task-b'],{id:'cmp-audit',label:'Audit comparison',now:'2026-08-08T21:02:00.000Z'}).project;
...
drift=structuredClone(project);drift.generationEvaluations[0].verdict='winner';audit=auditPromptStudioGenerationDecisions(drift);assert(!audit.ok&&audit.issues.some(item=>item.id==='evaluation-verdict-invalid'),'Audit must reject raw Evaluation verdicts that conflate review state with Winner decision.');

drift=structuredClone(project);drift.generationComparisons[0].taskIds=['task-a','task-a'];audit=auditPromptStudioGenerationDecisions(drift);assert(!audit.ok&&audit.issues.some(item=>item.id==='comparison-task-set-invalid'),'Audit must reject raw Comparisons that do not contain 2–8 unique tasks.');
...
drift=structuredClone(project);drift.generationWinners['cmp-audit'].comparisonId='other-comparison';audit=auditPromptStudioGenerationDecisions(drift);assert(!audit.ok&&audit.issues.some(item=>item.id==='winner-comparison-id-mismatch'),'Winner comparisonId must match its generationWinners map key.');
...
drift=structuredClone(project);drift.generationRetakes[0].lever='everything';audit=auditPromptStudioGenerationDecisions(drift);assert(!audit.ok&&audit.issues.some(item=>item.id==='retake-lever-invalid'),'Raw Retake must use a canonical one-lever registry value.');
...
drift=structuredClone(project);const sourceEval=drift.generationEvaluations.find(item=>item.taskId==='task-b');for(const dimension of Object.values(sourceEval.scores)){dimension.score=null;dimension.note='';dimension.evidence='';}audit=auditPromptStudioGenerationDecisions(drift);assert(!audit.ok&&audit.issues.some(item=>item.id==='retake-evaluation-evidence-thin'),'Retake source must retain at least one rated and one evidence-covered Evaluation dimension.');
...
if(failures.length){console.error('Prompt Studio v9 Decision Audit contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,cleanState:true,evaluationDrift:true,duplicateEvaluation:true,invalidVerdict:true,invalidComparisonSet:true,winnerOrphan:true,winnerKeyMismatch:true,winnerRationaleHardGate:true,winnerReadinessGate:true,retakeLeverRegistry:true,retakeLocksHardGate:true,retakeExpectedImprovementHardGate:true,retakeEvaluationEvidenceGate:true,batchLineageDrift:true,orphanDecisionDetection:true},null,2));
