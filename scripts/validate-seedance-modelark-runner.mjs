...
if(failures.length){console.error('Seedance ModelArk runner contract failed:\n'+failures.map(item=>`- ${item}`).join('\n'));process.exit(1);}console.log(JSON.stringify({ok:true,submit:true,resumableJob:true,polling:['running','succeeded'],terminalResult:true,cancelQueuedOnly:true,terminalDeleteRefused:true,downloadNoAuth:true,secretPersistence:false,upstreamSecretRedaction:true,networkGuard:true},null,2));
